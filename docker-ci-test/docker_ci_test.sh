...
docker run -v $PWD/mock-api-responses.md:/etc/secrets/api.md -p 3000:3000 -dit wolfdeng/api-mock-server
...
curl -i -u guest:guest -H "content-type:application/json" -XPOST -d @$PWD/docker-ci-rabbitmq-data.json http://localhost:15672/api/exchanges/%2f/amq.default/publish
